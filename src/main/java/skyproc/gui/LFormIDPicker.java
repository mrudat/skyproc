...
public class LFormIDPicker extends LUserSetting<FormID[]> {
	...
	LButton add;
	LButton remove;
	JComboBox<FormID> addPicker;
	DefaultListModel<FormID> model;
	JList<FormID> addedList;
	JScrollPane scroll;
	...
	public LFormIDPicker(String title, Font font, Color c) {
		...
		addPicker = new JComboBox<FormID>();
		...
		remove.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent arg0) {
				for (FormID o : addedList.getSelectedValuesList()) {
					model.removeElement(o);
				}
			}
		});
		add(remove);
		model = new DefaultListModel<FormID>();
		addedList = new JList<FormID>(model);
	...
	final public void setSize(int arg0, int arg1) {
		...
		remove.setSize(addPicker.getWidth() - add.getWidth() - 6,
				remove.getHeight());
