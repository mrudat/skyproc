...
public class SPImporter {
	 ...
	 * An SPImporter object should only be instantiated as an extended class
	 * that overrides the importControl() function.
	 */
	public SPImporter() {
	}
	 ...
	 * program to freeze while it processes. <br>
	 * <br>
	 * NOTE: You MUST override importControl() with custom code telling the
	 * program what you want it to do input the background.
	 */
	 ...
	 * input the results.<br>
	 * If any mods on the list are not present input the data folder, they will
	 * be skipped.<br>
	 * <br>
	 * If the program cannot locate plugins.txt, it will prompt the user to
	 * locate the file themselves. Once they do, it will create a file and use
	 * that as a reference for future patch generations.<br>
	 * <br>
	 * Related settings input SPGlobal:<br>
	 * - pluginsListPath<br>
	 * - pathToData<br>
	 ...
	 */
	static public ArrayList<ModListing> getActiveModList()
			throws java.io.IOException {
			...
			try {
				...
				while (line != null) {
					...
					if (!line.equals("")) {
						...
						if (SPGlobal.noModsAfter
								&& nextMod.equals(SPGlobal.getGlobalPatch()
										.getInfo())) {
							SPGlobal.logSync(header,
									"Skipping the remaining mods as they were after the patch.");
							break;
						} else if (SPGlobal.shouldImport(nextMod)
								&& SPGlobal.shouldImport(line)) {
							if (pluginName.isFile()) {
								if (Ln.indexOfIgnoreCase(lines, line) == -1) {
									SPGlobal.logSync(header, "Adding mod: "
											+ line);
									lines.add(line);
								} else if (SPGlobal.logging()) {
									SPGlobal.logSync(header,
											"Mod was already added: ", line);
								}
							} else if (SPGlobal.logging()) {
								SPGlobal.logSync(header, "Mod didn't exist: ",
										line);
							}
						} else if (SPGlobal.logging()) {
							SPGlobal.logSync(header,
									"Mod was on the list to skip: " + line);
						}
					}
	 ...
	 * then it will be omitted input the results.<br>
	 * <br>
	 * Related settings input SPGlobal:<br>
	 * - pathToData
	 *
	...
	static ArrayList<ModListing> sortModListings(ArrayList<String> lines) {
		...
		if (SPGlobal.logging()) {
			...
			for (ModListing m : listing) {
				SPGlobal.logSync(header, Ln.prettyPrintHex(counter++)
						+ " Name: " + m.print());
			}
		}
	 ...
	 * are interested input by using other import functions.
	 *
	 ...
	 * @see SPGlobal
	 * @param grup_targets
	 *            Any amount of GRUP targets, separated by commas, that you wish
	 *            to import.
	 ...
	 * @see SPGlobal
	 * @param grup_targets
	 *            An arraylist of GRUP_TYPE with the desired types to import
	 * @return A set of Mods with specified GRUPs imported and ready to be
	 *         manipulated.
	 * @throws MissingMaster
	 */
	 ...
	 * criteria of isModToSkip(), then it will be omitted input the results.<br>
	 * <br>
	 * If the program cannot locate plugins.txt, it will prompt the user to
	 * locate the file themselves. Once they do, it will create a file and use
	 * that as a reference for future patch generations.<br>
	 * <br>
	 * NOTE: It is suggested for speed reasons to only import the GRUP types you
	 * are interested input by using other import functions.<br>
	 * <br>
	 * Related settings input SPGlobal:<br>
	 * - pluginsListPath<br>
	 * - pathToData<br>
	 ...
	 * the criteria of isModToSkip(), then it will be omitted input the results.<br>
	 * <br>
	 * If the program cannot locate plugins.txt, it will prompt the user to
	 * locate the file themselves. Once they do, it will create a file and use
	 * that as a reference for future patch generations.<br>
	...
	static public Set<Mod> importActiveMods(GRUP_TYPE... grup_targets)
			throws IOException, MissingMaster {
	 ...
	 * specified input the parameter.<br>
	 * <br>
	 * Related settings input SPGlobal:<br>
	 * - pathToData<br>
	 *
	 * @see SPGlobal
	 * @param mods
	 *            ModListings to look for and import from the data folder.
	 * @param grup_targets
	 *            Any amount of GRUP targets, separated by commas, that you wish
	 *            to import.
	 * @return A set of Mods with specified GRUPs imported and ready to be
	...
	static public Set<Mod> importMods(ArrayList<ModListing> mods,
			GRUP_TYPE... grup_targets) throws MissingMaster {
	 ...
	 */
	static public Set<Mod> importMods(ArrayList<ModListing> mods,
			ArrayList<GRUP_TYPE> grup_targets) throws MissingMaster {
		GRUP_TYPE[] tmp = new GRUP_TYPE[0];
	 ...
	 *            ModListings to look for and import from the data folder.
	 * @param path
	 *            Path from patch location to where to load mods from.
	 * @return A set of Mods with all GRUPs imported and ready to be
	 *         manipulated.
	 * @throws MissingMaster
	 */
	...
	static public Set<Mod> importMods(final ArrayList<ModListing> mods,
		...
		if (grup_targets.length == 0) {
			SPGlobal.logMain(header,
					"Skipping import because requests were empty.");
			...
			if (!SPGlobal.modsToSkip.contains(new ModListing(mod))) {
				try {
					...
					SPGlobal.logError(header,
							"Exception occured while importing mod : " + mod);
	 ...
	 *
	 * @param listing
	 *            Mod name and suffix to look for.
	 * @param path
	 *            Path to look for the mod data.
	 * @param grup_targets
	 *            Any amount of GRUP targets, separated by commas, that you wish
	 *            to import.
	 ...
	 *         manipulated.
	 * @throws BadMod
	 *             If SkyProc runs into any unexpected data structures, or has
	 *             any error importing a mod at all.
	 * @throws MissingMaster
	 */
	static public Mod importMod(ModListing listing, String path,
			GRUP_TYPE... grup_targets) throws BadMod, MissingMaster {
	 ...
	 *            Path to look for the mod data.
	 * @param grup_targets
	 *            An ArrayList of GRUP targets that you wish to import.
	 * @return A mod with the specified GRUPs imported and ready to be
	 *         manipulated.
	 * @throws BadMod
	 *             If SkyProc runs into any unexpected data structures, or has
	 ...
	 *             ArrayList<GRUP_TYPE> grup_targets) throws BadMod {
	 *             GRUP_static Type[] types = new
	 *             GRUP_TYPE[grup_targets.size()]; types =
	 *             grup_targets.toArray(types); return importMod(listing, path,
	 *             types);
	 */
	static Mod importMod(ModListing listing, int index, String path,
			Boolean addtoDb, GRUP_TYPE... grup_targets) throws BadMod,
			MissingMaster {
		...
		SPGlobal.sync(true);
		try {
			RecordFileChannel input = new RecordFileChannel(path
					+ listing.print());
			Mod plugin = new Mod(listing, extractHeaderInfo(input));
			SPGlobal.logMod(plugin, header, "Opened filestream to mod: "
					+ listing.print());
			if (SPGlobal.isCheckMissingMasters()) {
			...
			while (iter.hasNext()) {
				...
				SPProgressBarPlug.setStatusNumbered(genStatus(listing) + ": "
						+ result);
				SPGlobal.logMod(plugin, header,
						"================== Loading in GRUP " + result + ": ",
						plugin.getName(), " ===================");
			...
			SPGlobal.logException(e);
			throw new BadMod(
					"Ran into an exception, check SPGlobal.logs for more details.");
		} finally {
			SPGlobal.sync(false);
		}
	}
	 ...
	 * @throws MissingMaster
	 */
	public static Mod importMod(ModListing listing, GRUP_TYPE... grup_targets)
			throws BadMod, MissingMaster {
	...
	static void checkMissingMasters(Mod plugin) throws MissingMaster {
		...
		for (ModListing master : plugin.getMasters()) {
			if (SPGlobal.getDB().getMod(master) == null
					&& SPGlobal.shouldImport(master)) {
				missingMasters.add(master);
			}
		}
		if (!missingMasters.isEmpty()) {
			String error = "\n" + plugin.toString()
					+ " has some missing masters:";
	 ...
	 * @return
	 */
	static public DirtyParsingIterator getSubRecordsInGRUPs(String typeString,
			String... grups) {
	 ...
	 * @return
	 */
	static public DirtyParsingIterator getSubRecordsInGRUPs(
			ModListing targetMod, String typeString, String... grups) {
	...
	static class GRUPIterator implements Iterator<RecordShrinkArray> {
		...
		GRUPIterator(GRUP_TYPE[] grup_targets, LInChannel input) {
			ArrayList<GRUP_TYPE> tmp = new ArrayList<>(
					Arrays.asList(grup_targets));
	 ...
	 * desired typestring.
	 */
	public static class DirtyParsingIterator implements
			Iterator<RecordShrinkArray> {
			...
			while (fileInput.available() >= 4 || uncompressed.available() >= 4) {
				...
				if ("TES4".equals(inputStr)) {
					...
					// System.out.println("Passed GRUP");
						...
						|| "REFR".equals(inputStr) || "ACHR".equals(inputStr)
						|| "LAND".equals(inputStr) || "NAVM".equals(inputStr)
						|| "NVNM".equals(inputStr)) {
					// If major record is target type
					if (typeString.equals(inputStr)) {
						next = new RecordShrinkArray(input.extract(input
								.extractInt(4) + 16));
						return;
					}
		...
		boolean switchToNextMod() {
			...
			if (activeMods.size() > 0) {
				fileInput = new LInChannel(SPGlobal.pathToData
						+ activeMods.get(0).print());
	...
	static ByteBuffer extractHeaderInfo(LInChannel in) {
		if (Ln.arrayToString(in.extractInts(0, 4)).equals("TES4")) {
			int size = Ln.arrayToInt(in.extractInts(0, 4)) + 24; // +24 for TES4
																	// extra
																	// info
	...
	static void importStringLocations(Mod mod) {
		...
		for (Files f : SubStringPointer.Files.values()) {
			try {
				...
				SPGlobal.logError(header, "Error Importing Strings " + f + ": "
						+ e);
			}
		}
	}
	...
	static void importStringLocations(Mod plugin, SubStringPointer.Files file)
		...
		if (in == null) {
			SPGlobal.logError(header, plugin.toString()
					+ " did not have Strings files (loose or in BSA).");
		} else {
			SPGlobal.logMod(plugin, header, "Loaded " + file
					+ " from language: " + plugin.language);
		}
	}

	static String getStringFilePath(Mod plugin, Language l,
			SubStringPointer.Files file) {
		return "Strings\\"
				+ plugin.getName()
						.substring(0, plugin.getName().indexOf(".es")) + "_"
				+ l + "." + file;
	}
