...
public class AltTextures extends SubRecordTyped {
	...
	void parseData(LImport in, Mod srcMod) throws BadRecord,
		...
		for (int i = 0; i < numTextures; i++) {
			...
			AltTexture newText = new AltTexture(new LShrinkArray(
					in.extract(12 + strLength)), srcMod);
			altTextures.add(newText);
			if (logging()) {
				logMod(srcMod, "", "New Texture Alt -- Name: " + newText.name
						+ ", texture: " + newText.texture + ", index: "
						+ newText.index);
			}
		}
	}
	 ...
	 * @return
	 */
	public static boolean equal(ArrayList<AltTexture> alts,
			ArrayList<AltTexture> rhsAlts) {
	...
	public static class AltTexture implements Serializable {
		 ...
		 *
		 * @param name
		 *            Name of the NiTriShape to apply this TXST to.
		 * @param txst
		 *            FormID of the TXST to apply as the alt.
		 * @param index
		 *            Index of the NiTriShape to apply this TXST to.
		 */
		 ...
		 *
		 * @param name
		 *            String to set the AltTexture name to.
		 */
		 ...
		 *
		 * @param txst
		 *            FormID of the TXST to tie the AltTexture to.
		 */
		 ...
		 *
		 * @param index
		 *            The NiTriShape index to assign.
		 */
