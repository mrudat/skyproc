...
public class PERK extends MajorRecordDescription {
	...
	static final SubPrototype PERKproto = new SubPrototype(
				...
				protected void addRecords() {
					...
					add(new PRKEComplexSubPackage(new SubPrototype() {
						...
						protected void addRecords() {
							...
							add(new SubStringPointer("EPF2",
									SubStringPointer.Files.STRINGS));
	...
	static class PRKEPackage extends SubShellBulkType {
		...
		final void parseData(LImport in, Mod srcMod) throws BadRecord,
			...
			PerkType perkType = PerkType.values()[subRecords.getSubData("PRKE")
					.getData()[0]];
			switch (perkType) {
			case QUEST:
		...
		@Override
		boolean isValid() {
			return subRecords.get("PRKE").isValid()
					&& subRecords.get("DATA").isValid();
		}
	}
