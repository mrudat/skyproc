...
public class BSA {
	...
	BSA(File file, boolean load) throws FileNotFoundException, IOException,
			BadParameter {
		this(file.getPath(), load);
	}

	BSA(String filePath, boolean load) throws FileNotFoundException,
			IOException, BadParameter {
		...
		if (SPGlobal.debugBSAimport && SPGlobal.logging()) {
			...
			SPGlobal.logSpecial(LogTypes.BSA, header, "| Offset " + offset
					+ ", archiveFlags: " + archiveFlags);
			SPGlobal.logSpecial(LogTypes.BSA, header,
					"| hasDirectoryNames: " + archiveFlags.get(0)
							+ ", hasFileNames: " + archiveFlags.get(1)
							+ ", compressed: " + archiveFlags.get(2));
			SPGlobal.logSpecial(LogTypes.BSA, header,
					"| FolderCount: " + Ln.prettyPrintHex(folderCount)
							+ ", FileCount: " + Ln.prettyPrintHex(fileCount));
			SPGlobal.logSpecial(
					LogTypes.BSA,
					header,
			...
			SPGlobal.logSpecial(LogTypes.BSA, header, "| fileFlags: "
					+ fileFlags.toString());
			SPGlobal.logSpecial(LogTypes.BSA, header, "|==================>");
		}
		if (load) {
	 ...
	 *
	 * @param filePath
	 *            Filepath to load BSA data from.
	 * @throws FileNotFoundException
	 * @throws IOException
	 * @throws BadParameter
	 *             If the BSA is malformed (by SkyProc standards)
	 */
	public BSA(String filePath) throws FileNotFoundException, IOException,
			BadParameter {
		this(filePath, true);
	}
	...
	final void loadFolders() {
		...
		if (SPGlobal.logging()) {
			SPGlobal.logSpecial(LogTypes.BSA, header,
					"|============================================");
			SPGlobal.logSpecial(LogTypes.BSA, header, "|============  Loading "
					+ this + " ============");
			SPGlobal.logSpecial(LogTypes.BSA, header,
					"|============================================");
		}
		try {
			...
			int fileCounter = 0;
			in.pos(offset);
			LShrinkArray folderData = new LShrinkArray(in.extract(0,
					folderCount * 16));
			posAtFilenames();
			LShrinkArray fileNames = new LShrinkArray(in.extract(0,
					fileNameLength));
			for (int i = 0; i < folderCount; i++) {
				...
				if (SPGlobal.debugBSAimport && SPGlobal.logging()) {
					SPGlobal.logSpecial(LogTypes.BSA, header, "Loaded folder: "
							+ folder.name);
				}
			...
			if (SPGlobal.logging()) {
				if (SPGlobal.debugBSAimport) {
					SPGlobal.logSpecial(LogTypes.BSA, header, "Loaded "
							+ fileCounter + " files.");
				}
				SPGlobal.logSpecial(LogTypes.BSA, header, "Loaded BSA: "
						+ getFilePath());
			}
		} catch (Exception e) {
	 ...
	 *
	 * @param filePath
	 *            filepath to query for and retrieve.
	 * @return ShrinkArray of the raw data from the BSA of the file specified,
	 *         already decompressed if applicable; Empty ShrinkArray if the file
	 *         did not exist.
	 * @throws IOException
	 * @throws DataFormatException
	 */
	public LShrinkArray getFile(String filePath) throws IOException,
			DataFormatException {
		BSAFileRef ref;
	...
	static public LShrinkArray getUsedFile(String filePath) throws IOException,
		...
		if (outsideBSA.isFile()) {
			SPGlobal.logSpecial(LogTypes.BSA, header,
					"Loaded from loose files: " + outsideBSA.getPath());
			...
			if (bsa != null) {
				if (SPGlobal.logging()) {
					SPGlobal.logSpecial(LogTypes.BSA, header,
							"Loaded from BSA " + bsa.getFilePath() + ": "
									+ filePath);
				}
	...
	static void loadPluginLoadOrder() {
		...
		if (SPGlobal.logging()) {
			SPGlobal.logSpecial(LogTypes.BSA, header,
					"Loading in active plugin BSA headers.");
		}
		try {
	...
	static synchronized void loadResourceLoadOrder() {
		...
		try {
			...
			try {
				...
				if (SPGlobal.logging()) {
					SPGlobal.logSpecial(LogTypes.BSA, header,
							"Loading in BSA list from Skyrim.ini: " + ini);
				}
				...
				while (input.available() > 0
						&& !line.toUpperCase().contains("SRESOURCEARCHIVELIST")) {
			...
			if (SPGlobal.logging()) {
				SPGlobal.logSpecial(LogTypes.BSA, header,
						"BSA resource load order: ");
				for (String s : resources) {
					SPGlobal.logSpecial(LogTypes.BSA, header, "  " + s);
				}
				SPGlobal.logSpecial(LogTypes.BSA, header,
						"Loading in their headers.");
			}
			...
			for (String s : resources) {
				...
				if (bsaPath.exists()) {
					...
					SPGlobal.logSpecial(LogTypes.BSA, header,
							"  BSA skipped because it didn't exist: " + bsaPath);
				}
			}
	...
	public Map<String, ArrayList<String>> getFiles() {
		...
		for (BSAFolder folder : folders.values()) {
			ArrayList<String> list = new ArrayList<>(folder.files.values()
					.size());
	...
	static ArrayList<BSA> getBSAs() {
		ArrayList<BSA> order = new ArrayList<>(resourceLoadOrder.size()
				+ pluginLoadOrder.size());
	...
	static public BSA getBSA(ModListing m) {
		...
		File bsaPath = new File(SPGlobal.pathToData
				+ Ln.changeFileTypeTo(m.print(), "bsa"));
		if (bsaPath.exists()) {
			try {
				BSA bsa = new BSA(bsaPath, false);
				pluginLoadOrder.put(m, bsa);
