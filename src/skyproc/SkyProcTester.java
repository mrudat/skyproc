...
public class SkyProcTester {
	...
	static ArrayList<GRUP_TYPE> skip = new ArrayList<>(
			Arrays.asList(new GRUP_TYPE[] { GRUP_TYPE.BOOK }));
	...
	public static void runTests(int test) {
		...
		SPDefaultGUI gui = new SPDefaultGUI("Tester Program",
	...
	private static boolean test(GRUP_TYPE type, ModTestPackage p)
			throws IOException, BadRecord, BadMod {
		...
		passed = passed
				&& NiftyFunc.validateRecordLengths(SPGlobal.pathToData
						+ "Test.esp", 10);
		File validF = new File("Validation Files/" + type.toString() + "_"
				+ p.main.printNoSuffix() + ".esp");
		if (validF.isFile()) {
			passed = Ln.validateCompare(SPGlobal.pathToData + "Test.esp",
					validF.getPath(), 10) && passed;
		} else {
			System.out
					.println("Didn't have a source file to validate bytes to.");
		}
	...
	public static boolean copyTest() throws IOException, BadRecord {
		SPProgressBarPlug.pause(true);
