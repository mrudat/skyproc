...
public class SPDefaultGUI extends JFrame {
	 ...
	 *
	 * @param yourPatcherName
	 *            This will be used as the title on the GUI.
	 * @param yourDescription
	 *            This will be displayed under the title.
	 */
	...
	final void init(String pluginName, String descriptionText) {
		try {
			...
			Dimension descSize = new Dimension(this.getWidth() - 100, this.getHeight() - descY - (this.getHeight() - patching.getY()) - 15);
	 ...
	 *
	 * @param c
	 *            GUI component to add to the default GUI
	 ...
	 * @param logo
	 *            URL to an image.
	 * @param descriptionOffset
	 *            Y-offset to give the description box, to help align where it
	 *            should begin in relation to your image.
	 * @throws IOException
	 *             If the image given by the URL cannot be loaded properly.
	 */
	public void replaceHeader(final URL logo, final int descriptionOffset) throws IOException {
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				patcherLogo.setLocation(getWidth() / 2 - patcherLogo.getWidth() / 2, 0);
	...
	void fitDesc(final Component descAnchor, final int offset) {
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				Dimension descSize = new Dimension(getWidth() - 100, getHeight() - descY - (getHeight() - patching.getY()) - 15 - offset);
