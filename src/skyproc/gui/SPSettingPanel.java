...
public abstract class SPSettingPanel extends LPanel {
	 ...
	 * @param headerColor
	 */
	public SPSettingPanel(SPMainMenuPanel parent_, String title,
			Color headerColor) {
	...
	protected void initialize() {
		...
		scroll.setSize(SUMGUI.middleDimensions.width,
				SUMGUI.middleDimensions.height);
		add(scroll);

		header.addShadow();
		header.setLocation(
				settingsPanel.getWidth() / 2 - header.getWidth() / 2, 20);
	 ...
	 *            Button on right
	 * @param left
	 *            To return spacing for left button?
	 * @return Spacing to be given target button.
	 */
	protected Point getSpacing(LButton in1, LButton in2, boolean left) {
		int spacing = (settingsPanel.getWidth() - in1.getWidth() - in2
				.getWidth()) / 3;
		if (left) {
			return new Point(spacing, settingsPanel.getHeight()
					- in1.getHeight() - 15);
		} else {
			return new Point(in1.getX() + in1.getWidth() + spacing,
					settingsPanel.getHeight() - in2.getHeight() - 15);
		}
	}
	...
	public final void Add(Component c) {
		...
		if (range > settingsPanel.getPreferredSize().getHeight()) {
			settingsPanel.setPreferredSize(new Dimension(settingsPanel
					.getPreferredSize().width, range));
		}
	}
