...
public class RACE extends MajorRecordDescription {
	...
	static final SubPrototype RACEproto = new SubPrototype(
		...
		protected void addRecords() {
			...
			add(new KeywordSet());
			add(new DATA());
			SubMarkerSet mfnam = new SubMarkerSet<>(new SubShell(
					new SubPrototype() {
						@Override
						protected void addRecords() {
			...
			add(new SubShellBulkType(new SubPrototype() {
				...
				protected void addRecords() {
					...
					SubMarkerSet BodyData = new SubMarkerSet(new SubShell(
							new SubPrototype() {
								@Override
								protected void addRecords() {
									add(new SubData("INDX"));
	 ...
	 *
	 * @param id
	 *            FormID to set the worn ARMO record to.
	 */
	...
	public void addFaceDetailsTexture(Gender g, FormID id) {
		switch (g) {
		case FEMALE:
			getHeadData(g).subRecords.getSubList("FTSF").add(
					new SubForm("FTSF", id));
			break;
		default:
			getHeadData(g).subRecords.getSubList("FTSM").add(
					new SubForm("FTSM", id));
		}
	}
	...
	public void removeFaceDetailsTexture(Gender g, FormID id) {
		switch (g) {
		case FEMALE:
			getHeadData(g).subRecords.getSubList("FTSF").remove(
					new SubForm("FTSF", id));
			break;
		default:
			getHeadData(g).subRecords.getSubList("FTSM").remove(
					new SubForm("FTSM", id));
		}
	}
