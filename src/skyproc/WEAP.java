...
public class WEAP extends MajorRecordDescription {
	...
	static final SubPrototype WEAPproto = new SubPrototype(
	...
	static final class DNAM extends SubRecord {
		...
		void parseData(LImport in, Mod srcMod) throws BadRecord,
			...
			if (logging()) {
				logMod(srcMod, "", "WType: " + wtype + ", speed: " + speed
						+ ", reach: " + reach);
				logMod(srcMod, "", "SightFOV: " + sightFOV + ", vats: " + vats
						+ ", numProjectiles: " + numProjectiles);
				logMod(srcMod, "", "EmbeddedWeapActorVal: "
						+ embeddedWeapActorValue + ", MinRange: " + minRange
						+ ", MaxRange: " + maxRange);
				logMod(srcMod, "", "stagger: " + stagger + ", Bound: "
						+ get(WeaponFlag.BoundWeapon) + ", Cant Drop: "
						+ get(WeaponFlag.CantDrop));
				logMod(srcMod, "", "Hide Backpack: "
						+ get(WeaponFlag.HideBackpack)
						+ ", Ignore Normal Weapon Resistance: "
						+ get(WeaponFlag.IgnoresNormalWeaponResistance)
						+ ", Minor Crime: " + get(WeaponFlag.MinorCrime));
				logMod(srcMod, "", "NPCs Use Ammo: "
						+ get(WeaponFlag.NPCsUseAmmo)
						+ ", No jam after reload: "
						+ get(WeaponFlag.NoJamAfterReload) + ", Non Hostile: "
						+ get(WeaponFlag.NonHostile));
				logMod(srcMod, "", "Non Playable: "
						+ get(WeaponFlag.NonPlayable)
						+ ", Not used in normal combat: "
						+ get(WeaponFlag.NotUsedInNormalCombat)
						+ ", Player Only: " + get(WeaponFlag.PlayerOnly));
			}
		}
	...
	static final class CRDT extends SubRecord {
		...
		void parseData(LImport in, Mod srcMod) throws BadRecord,
			...
			if (logging()) {
				logMod(srcMod, "", "critDmg: " + critDmg + ", critMult: "
						+ critMult + ", crit effect: " + critEffect);
			}
		}
	...
	/**
	 * Returns the top of the Weapon Template "chain" and returns the top (the
	 * one without any template). Only returns null if a template formID is
	 * found, but no record exists with that formid.
	 * 
	 * @return
	 */
	public WEAP getTemplateTop() {
		if (!isTemplated()) {
			...
			WEAP template = (WEAP) SPDatabase.getMajor(getTemplate(),
					GRUP_TYPE.WEAP);
			if (template != null) {
