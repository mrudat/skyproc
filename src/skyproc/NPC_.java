...
public class NPC_ extends MajorRecordNamed implements Serializable {
	...
	static final SubPrototype NPC_proto = new SubPrototype(
				...
				for (Skill s : Skill.values()) {
					...
					logMod(srcMod,
							...
							"  "
									...
									+ Ln.spaceLeft(
											false,
											15 - s.toString().length()
													+ temp.length(), ' ', temp));
				}
				logMod(srcMod, "", "  " + "Health: " + health + ", Magicka: "
						+ magicka + ", Stamina: " + stamina);
				logMod(srcMod, "", "  " + "Far Away Distance: "
						+ farAwayDistance + ", Geared Up weapons: "
						+ gearedUpWeapons);
			}
		}
	...
	static class ACBS extends SubRecord<ACBS> implements Serializable {
		...
		final void parseData(LImport in, Mod srcMod) throws BadRecord,
			...
			if (logging()) {
				...
				logMod(srcMod, "", "  " + "Base Spell Points: " + magickaOffset
						+ ", Base Fatigue: " + fatigueOffset);
				logMod(srcMod, "", "  " + "Level: " + level
						+ ", Min Calculated Level: " + minCalcLevel
						+ ", Max Calculated Level: " + maxCalcLevel);
				logMod(srcMod, "", "  " + "Speed Multiplier: " + speed
						+ ", Disposition Base: " + dispositionBase);
			}
		}
	...
	static class AIDT extends SubRecord<AIDT> implements Serializable {
		...
		final void parseData(LImport in, Mod srcMod) throws BadRecord,
			...
			if (logging()) {
				...
				logMod(srcMod, "", "  Aggression: " + aggression
						+ ", Confidence: " + confidence + ", Morality: "
						+ morality);
				logMod(srcMod, "", "  Assistance: " + assistance + ", Mood: "
						+ mood + ", AggroRadiusBehavior: "
						+ aggroRadiusBehavior);
				logMod(srcMod, "", "  Aggro Attack: " + aggroAttack
						+ ", Aggro Warn: " + aggroWarn
						+ ", Aggro Warn/Attack: " + aggroWarnAttack);
			}
		}
	...
	public enum NPCStat {
		...
		DISPOSITION_BASE,
		/**
		 * Determines the fatigue of the NPC.<br>
		...
		SPEED_MULT,
		/**
		 * Determines the initial mana of the NPC.<br>
		 * Not confirmed whether this is actually used.
		 */
		SPELL_POINTS_BASE
	}
	 ...
	 * input flags. It also unchecks the specific template flags on the NPC.<br>
	 * <br>
	 *
	 ...
	 * instead mark the flag on the NPC (if it wasn't already).<br>
	 * <br>
	 *
	 * If no template flags remain checked after this function has run, then the
	 * NPC's template reference is set to NULL.<br>
	 * <br>
	 * This function makes a deep copy of all templated info.
	 *
	 * @param otherNPC
	 *            NPC FormID to assume info from.
	 * @param flags
	...
	boolean templateToInternal(NPC_ otherNPC, TemplateFlag flag) {
		...
		if (otherNPC.get(flag)) {
			NPC_ otherNPCsTemplate = (NPC_) SPDatabase.getMajor(
					otherNPC.getTemplate(), GRUP_TYPE.NPC_);
			...
			case USE_TRAITS:
				...
				set(NPCFlag.OppositeGenderAnims,
						otherNPC.get(NPCFlag.OppositeGenderAnims));
				// Sound Tab
			...
			case USE_STATS:
				...
				set(NPCStat.MIN_CALC_LEVEL,
						otherNPC.get(NPCStat.MIN_CALC_LEVEL));
				set(NPCStat.MAX_CALC_LEVEL,
						otherNPC.get(NPCStat.MAX_CALC_LEVEL));
			...
			case USE_AI_DATA:
				...
				this.set(NPCFlag.AggroRadiusBehavior,
						otherNPC.get(NPCFlag.AggroRadiusBehavior));
			...
			case USE_BASE_DATA:
				...
				this.set(NPCFlag.DoesntAffectStealthMeter,
						otherNPC.get(NPCFlag.DoesntAffectStealthMeter));
				break;
			case USE_KEYWORDS:
	 ...
	 * <br>
	 * Flags can be specified if you only want to return a Leveled List if AT
	 * LEAST one of those flags is checked.
	 *
	 * @param templateFlagsToCheck
	 *            Flags to consider. If none are given, then all considered.
	 ...
	 *
	 * @param flag
	 *            Template flag to set.
	 * @param on
	 *            What to set the template flag to.
	 */
	 ...
	 * 
	 * @param factionRef
	 *            FormID of the faction to add the NPC into.
	 * @param rank
	 *            Rank within the faction to set the NPC at.
	 * @return True if faction was added.
	 */
	public boolean addFaction(FormID factionRef, int rank) {
		return subRecords.getSubList("SNAM").add(
				new SubFormInt("SNAM", factionRef, rank));
	}

	/**
	 * 
	 * @param factionRef
	 *            FormID matching the FactionRef record to remove.
	 * @return True if faction was removed.
	 */
	public boolean removeFaction(FormID factionRef) {
		return subRecords.getSubList("SNAM").remove(
				new SubFormInt("SNAM", factionRef, 0));
	}
	...
	public void addPerk(FormID perkRef, int rank) {
		subRecords.getSubList("PRKR")
				.add(new SubFormInt("PRKR", perkRef, rank));
	}
	...
	public boolean removePerk(FormID perkRef) {
		return subRecords.getSubList("PRKR").remove(
				new SubFormInt("PRKR", perkRef, 0));
	}
	 ...
	 * @see Skills
	 * @param skill
	 *            The enum of the skill to return the base value of.
	 ...
	 * @see Skills
	 * @param skill
	 *            The enum of the skill to set to the value.
	 * @param value
	 *            Sets the base value of the skill to this value.
	 */
	 ...
	 * @see Skills
	 * @param skill
	 *            The enum of the skill to return the mod value of.
	 ...
	 *            The enum of the skill to set to the value.
	 * @param value
	 *            Sets the mod value of the skill to this value.
	 */
	 ...
	 * @see Stat_Values
	 * @param stat
	 *            The enum of the stat data to return.
	 ...
	 * @see Stat_Values
	 * @param stat
	 *            The enum of the stat data to set to the value.
	 * @param value
	 *            Sets the value of the stat data to this value.
	 */
	...
	/**
	 * 
	 * @param spellReference
	 *            FormID of the spell to give to the NPC.
	 ...
	 * 
	 * @param spellReference
	 *            FormID of the spell to remove from the NPC
	...
	public boolean addItem(FormID itemReference, int count) {
		return subRecords.getSubList("CNTO").add(
				new ItemListing(itemReference, count));
	}
	...
	public boolean removeItem(FormID itemReference) {
		return subRecords.getSubList("CNTO").remove(
				new ItemListing(itemReference));
	}
